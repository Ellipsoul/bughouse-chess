...
export function parseChessComCompressedMovelist(s: string): string[] {
    ...
    for (var i = 0; i < moves_by_two.length; i++) {
        var so = moves_by_two[i][0];
        var ta = moves_by_two[i][1];
        ...
        if ("{~}(^)[_]@#$".includes(ta)) {
            // Handling promotions
            var promoteTo = "qnrb"[Math.floor("{~}(^)[_]@#$".indexOf(ta) / 3)];
            var fileChange = ("{~}(^)[_]@#$".indexOf(ta) % 3) - 1;
            var newRank = (sq(so)[1] === 7) ? 8 : 1;
            var newFile = String.fromCharCode(sq(so).charCodeAt(0) + fileChange);
            var promotionTarget = newFile + newRank;
            brd[so] = ".";
            brd[promotionTarget] = promoteTo.toUpperCase();

            if (sq(so)[0] !== newFile) {
                mv.push(sq(so)[0] + "x" + promotionTarget + "=" + promoteTo.toUpperCase());
            } else {
