...
describe("validateAndApplyBughouseHalfMove - captureMaterial", () => {
  it("tracks a regular capture (pawn = 1)", () => {
    ...
    expect(result.next.captureMaterial.A.white).toBe(1);
    expect(result.next.captureMaterial.A.black).toBe(-1);
  });

  it("tracks an en-passant capture (pawn = 1)", () => {
    const pos: BughousePositionSnapshot = {
  ...
  it("treats capturing a promoted piece as capturing a pawn (1)", () => {
    const pos: BughousePositionSnapshot = {
