/**
 * Migration Script: Shared Game Hashes (SHA-256)
 *
 * Recomputes content hashes for existing shared games using the SHA-256-based
 * hash format and stores them in users/{userId}/sharedGames/{sharedId}.
 *
 * Prerequisites:
 ...
 * Usage:
 *   # Dry run (default)
 *   npm run migration:shared-game-hashes-sha256
 *
 *   # Apply changes
 *   DRY_RUN=false npm run migration:shared-game-hashes-sha256
 */
...
/**
 * Recomputes SHA-256 content hashes and updates user index docs when values differ.
 * Skips documents whose hash already matches.
 */
async function runMigration(dryRun: boolean): Promise<MigrationStats> {
  ...
  log("Starting shared game hash migration (SHA-256)...");
  log(`Dry run: ${dryRun}`);
  log("");
  ...
  for (const docSnap of sharedGamesSnapshot.docs) {
    ...
    try {
      ...
      const userSharedGameSnap = await userSharedGameRef.get();
      if (!userSharedGameSnap.exists) {
        throw new Error("Missing user shared game reference document");
      }

      const existingHash = (userSharedGameSnap.data() as { contentHash?: string }).contentHash;
      ...
      logError(`Failed to migrate ${sharedId}: ${message}`);
    }
  }
...
async function main(): Promise<void> {
  ...
  console.log("Shared Game Hash Migration (SHA-256)");
  console.log("=".repeat(80));
  console.log("");
